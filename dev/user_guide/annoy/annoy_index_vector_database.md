# Vector Similarity Search and Vector Database[#](#vector-similarity-search-and-vector-database "Link to this heading")
...
## Vector similarity search[#](#vector-similarity-search "Link to this heading")
...
### Distance and similarity metrics[#](#distance-and-similarity-metrics "Link to this heading")
...
* ****Hamming distance**** counts how many positions differ.
  ...
  * It is the number of indices where \(u\_i \neq v\_i\).
...
> **Note**
> Dot product is **not** scale-invariant: if you multiply \(\mathbf{u}\) by 2,
the dot product doubles. Using \(d\_{\text{dot}} = -(\mathbf{u}\cdot\mathbf{v})\)
produces the same ranking as maximizing \(\mathbf{u}\cdot\mathbf{v}\) (it just
flips the sign). If vectors are L2-normalized, dot product and cosine similarity
become equivalent.

* ****Dot product (similarity score)****:

  \[s\_{\text{dot}}(\mathbf{u}, \mathbf{v})
  = \mathbf{u} \cdot \mathbf{v}
  = \sum\_{i=1}^{k} u\_i v\_i\]
* ****Negative dot product**** (dot as a distance to minimize):

  \[d\_{\text{dot}}(\mathbf{u}, \mathbf{v})
  = -(\mathbf{u} \cdot \mathbf{v})
  = -\sum\_{i=1}^{k} u\_i v\_i\]
* ****Cosine similarity****:
...
* ****Cosine distance**** (common definition):
  ...
  \[d\_{\text{cos}}(\mathbf{u}, \mathbf{v})
  = 1 - \text{cos\\_sim}(\mathbf{u}, \mathbf{v})\]
* ****Euclidean (L2) distance****:

  \[d\_{2}(\mathbf{u}, \mathbf{v})
  = \sqrt{\sum\_{i=1}^{k} (u\_i - v\_i)^2}\]
* ****Manhattan (L1) distance****:

  \[d\_{1}(\mathbf{u}, \mathbf{v})
  = \sum\_{i=1}^{k} \lvert u\_i - v\_i \rvert\]
* ****Hamming distance****:

  \[d\_{\text{ham}}(\mathbf{u}, \mathbf{v})
  = \sum\_{i=1}^{k} \mathbb{1}[u\_i \neq v\_i]\]

## Vector database vs “vector index library”[#](#vector-database-vs-vector-index-library "Link to this heading")
...
## Pros and cons of vector search[#](#pros-and-cons-of-vector-search "Link to this heading")
...
## 5 practical tips[#](#practical-tips "Link to this heading")
...
## Open source options[#](#open-source-options "Link to this heading")
...
## How to choose (simple rules)[#](#how-to-choose-simple-rules "Link to this heading")
