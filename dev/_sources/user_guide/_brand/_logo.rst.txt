...
.. note::
   ...
   The public interface is exposed as :mod:`~scikitplot._brand._logo`.
   The internal implementation may live in :mod:`~scikitplot._brand._logo`.

   If you are developing inside the repository you may see examples using
   ``sp._brand._logo``; user code should prefer ``sp._brand._logo`` when available.
...
.. prompt:: python >>>
   ...
   sp._brand._logo.save(
       ...
       variants=sp._brand._logo.list_variants()
   )
...
.. code-block:: python
   ...
   sp._brand._logo.save("assets/scikit-plots.svg", variants=sp._brand._logo.list_variants())
...
.. code-block:: python
   ...
   sp._brand._logo.save("assets/scikit-plots-dark.svg", theme="dark")

   # Monochrome for badges
   sp._brand._logo.save("assets/scikit-plots-mono.svg", mono=True)
...
.. code-block:: python
   ...
   sp._brand._logo.save("assets/scikit-plots-primary.svg", dots="fixed")

   # Turn off dots for extra minimal output
   sp._brand._logo.save("assets/scikit-plots-clean.svg", dots="none")

   # Experimental random layout (not recommended for official assets)
   sp._brand._logo.save("assets/scikit-plots-random.svg", dots="random", seed=42)
...
.. code-block:: python
   ...
   sp._brand._logo.save("assets/favicon.png", preset="favicon")

   # A balanced default for social/GitHub avatars
   sp._brand._logo.save("assets/avatar.svg", preset="avatar")

   # A large icon for docs landing pages
   sp._brand._logo.save("assets/hero.svg", preset="docs-hero")
...
.. code-block:: python
   ...
   sp._brand._logo.wordmark.save("assets/scikit-plots-lockup.svg")
...
.. code-block:: python
   ...
   sp._brand._logo.save("assets/scikit-plots.png", dpi=300)

   # Force SVG even without suffix
   sp._brand._logo.save("assets/scikit-plots", format="svg")
...
.. code-block:: bash
   ...
   python -m scikitplot._brand._logo --all --out assets

   # Generate a favicon preset
   python -m scikitplot._brand._logo --all --preset favicon --out assets --format png
   python -m scikitplot._brand._logo --preset favicon --out assets --format png

   # Generate the wordmark lockup
   python -m scikitplot._brand._logo --wordmark --out assets --format svg
   python -m scikitplot._brand._logo --wordmark --out assets --format png
...
.. seealso::
   ...
   * :func:`scikitplot._brand._logo.create`
   * :func:`scikitplot._brand._logo.save`
   * :func:`scikitplot._brand._logo.list_variants`
