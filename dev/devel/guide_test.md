# Testing Guidelines[#](#testing-guidelines "Link to this heading")

This section describes the [pytest](https://docs.pytest.org/en/stable/) testing framework and format standards for tests in
Astropy core, coordinated packages, and packages using the [OpenAstronomy Packaging Guide](https://packaging-guide.openastronomy.org/en/latest).
It also serves as recommendations for affiliated packages.

## Testing Dependencies[#](#testing-dependencies "Link to this heading")
...
This will provide all dependencies for running the full test suite using [tox](https://tox.wiki/)
and [pytest](https://docs.pytest.org/en/stable/). It will also allow running tests via any IDE which
...
`pytest`. Each of these invokes [pytest](https://docs.pytest.org/en/stable/) to run
...
to make use of [Tox](https://tox.readthedocs.io/en/latest/), which is a
...
pre-commit hook, as described in the [Pre-commit](guide_devel_details.html#pre-commit) section.
...
[pytest-filter-subpackage](https://github.com/astropy/pytest-filter-subpackage)
...
### pytest[#](#running-pytest "Link to this heading")
...
For more details, see the [pytest invocation guide](https://docs.pytest.org/en/stable/how-to/usage.html) and the
description of [caching](https://docs.pytest.org/en/stable/how-to/cache.html).

### Test-running options[#](#test-running-options "Link to this heading")

#### Testing for open files[#](#testing-for-open-files "Link to this heading")
...
#### Test coverage reports[#](#test-coverage-reports "Link to this heading")

Coverage reports can be generated using the [pytest-cov](https://pypi.org/project/pytest-cov/) plugin (which is installed
...
#### Running tests in parallel[#](#running-tests-in-parallel "Link to this heading")

It is possible to speed up astropy’s tests using the [pytest-xdist](https://pypi.org/project/pytest-xdist) plugin.
...
#### Running tests on an installed `astropy`[#](#running-tests-on-an-installed-astropy "Link to this heading")
...
Consult the [test discovery rules](https://pytest.org/en/stable/explanation/goodpractices.html#python-test-discovery "(in pytest v9.1.1)")
...
### Where to put tests[#](#where-to-put-tests "Link to this heading")

#### Package-specific tests[#](#package-specific-tests "Link to this heading")
...
#### Interoperability tests[#](#interoperability-tests "Link to this heading")
...
### Regression tests[#](#regression-tests "Link to this heading")
...
### Working with data files[#](#working-with-data-files "Link to this heading")
...
[pytest-remotedata](https://pypi.org/project/pytest-remotedata/).
...
#### Tests that use the file cache[#](#tests-that-use-the-file-cache "Link to this heading")
...
The [pytest ‘tmp\_path’ fixture](https://pytest.org/en/stable/how-to/tmp_path.html#tmp-path "(in pytest v9.1.1)") allows for the
...
### Setting up/Tearing down tests[#](#setting-up-tearing-down-tests "Link to this heading")
...
#### Method-level setup/teardown[#](#method-level-setup-teardown "Link to this heading")
...
#### Function-level setup/teardown[#](#function-level-setup-teardown "Link to this heading")
...
### Property-based tests[#](#property-based-tests "Link to this heading")
...
`pytest-astropy` includes a dependency on [Hypothesis](https://hypothesis.readthedocs.io/), so installation is easy -
you can just read the docs or [work through the tutorial](https://github.com/Zac-HD/escape-from-automanual-testing/)
...
bugs in time handling. See issue [#9017](https://github.com/astropy/astropy/issues/9017)
and pull request [#9532](https://github.com/astropy/astropy/pull/9532) for details!

(and if you find Hypothesis useful in your research,
[please cite it](https://doi.org/10.21105/joss.01891)!)
...
### Tests requiring optional dependencies[#](#tests-requiring-optional-dependencies "Link to this heading")
...
### Testing warnings[#](#testing-warnings "Link to this heading")
...
[pytest.warns](https://pytest.org/en/stable/how-to/capture-warnings.html#warns "(in pytest v9.1.1)") that, completely
...
[recwarn](https://pytest.org/en/stable/how-to/capture-warnings.html#recwarn "(in pytest v9.1.1)") function argument to test that
...
### Testing exceptions[#](#testing-exceptions "Link to this heading")
...
[pytest.raises](https://pytest.org/en/stable/how-to/assert.html#assertraises "(in pytest v9.1.1)")
...
[flag](https://docs.python.org/3/library/re.html#re-syntax "(in Python v3.14)")
...
### Testing configuration parameters[#](#testing-configuration-parameters "Link to this heading")
...
### Marking blocks of code to exclude from coverage[#](#marking-blocks-of-code-to-exclude-from-coverage "Link to this heading")
...
### Image tests with pytest-mpl[#](#image-tests-with-pytest-mpl "Link to this heading")

#### Running image tests[#](#running-image-tests "Link to this heading")
...
#### Writing image tests[#](#writing-image-tests "Link to this heading")

The [README.rst](https://github.com/matplotlib/pytest-mpl/blob/master/README.rst)
...
#### Failing tests[#](#failing-tests "Link to this heading")
...
#### New hash libraries[#](#new-hash-libraries "Link to this heading")
...
the case, follow the instructions in [Failing tests](#failing-tests) to update the hashes.
...
in [Failing tests](#failing-tests).

#### Generating reference images[#](#generating-reference-images "Link to this heading")
...
update the reference images in the [astropy-figure-tests](https://github.com/astropy/astropy-figure-tests)
repository.

## Writing doctests[#](#writing-doctests "Link to this heading")
...
Furthermore, Python includes a [`doctest`](https://docs.python.org/3/library/doctest.html#module-doctest "(in Python v3.14)") module that can detect these
...
write them, see the full [`doctest`](https://docs.python.org/3/library/doctest.html#module-doctest "(in Python v3.14)") documentation.
...
### Skipping doctests[#](#skipping-doctests "Link to this heading")
...
### Skipping output[#](#skipping-output "Link to this heading")
...
of the output are checked. See the [`doctest`](https://docs.python.org/3/library/doctest.html#module-doctest "(in Python v3.14)") documentation for
more examples of skipping output.

#### Ignoring all output[#](#ignoring-all-output "Link to this heading")
...
### Handling float output[#](#handling-float-output "Link to this heading")
...
## Continuous integration[#](#continuous-integration "Link to this heading")

### Overview[#](#overview "Link to this heading")

Astropy uses the following continuous integration (CI) services:

* [GitHub Actions](https://github.com/astropy/astropy/actions) for
  ...
  it anyway!)
* [CircleCI](https://circleci.com) for visualization tests
...
Maintainers have the option to run [comparative benchmark](maintainers/maintainer_workflow.html#benchmarks) using GitHub Actions
to test a new pull request against the current `main` branch. It uses the benchmarks
from [astropy-benchmarks](https://github.com/astropy/astropy-benchmarks/).
