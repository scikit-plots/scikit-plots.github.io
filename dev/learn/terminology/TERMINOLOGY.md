# scikit-plots `learn/terminology` — Regeneration & Maintenance Guide[#](#scikit-plots-learn-terminology-regeneration-maintenance-guide "Link to this heading")
...
### The 26 themes (grouped by level, in registry order)[#](#the-26-themes-grouped-by-level-in-registry-order "Link to this heading")
...
## 5. Updating & extending (add or edit terms)[#](#updating-extending-add-or-edit-terms "Link to this heading")

### Edit an existing term[#](#edit-an-existing-term "Link to this heading")

1. Edit `CONTENT["<exact title>"]` (and/or `MINDMAP["<exact title>"]`) in `term_content.py`.
2. `python build_terminology.py` → 4. validate.

### Add a new term[#](#add-a-new-term "Link to this heading")
...
## 6. Page content contract (what a good page contains)[#](#page-content-contract-what-a-good-page-contains "Link to this heading")
...
* ****Dollar signs:**** a bare `$` in prose can trigger math parsing — wrap amounts in `inline code` or spell out “dollars”. A bare `%` renders fine. A single unpaired `$` inside a **title**
