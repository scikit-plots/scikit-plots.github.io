# Terminology Page — Migration & Review Guide[#](#terminology-page-migration-review-guide "Link to this heading")
...
## 0. TL;DR — what to do, in order[#](#tl-dr-what-to-do-in-order "Link to this heading")
...
2. ****Adopt the per-term contract**** (Section 3.2) and the ****sphinx\_design templates**** (Section 3.3) verbatim so every future term looks identical.
...
## 1. Goal & non-goals[#](#goal-non-goals "Link to this heading")

### Goal[#](#goal "Link to this heading")
...
* exposes ****sphinx\_tags**** so the page is discoverable via the existing tag pages and search.

### Non-goals (explicitly out of scope)[#](#non-goals-explicitly-out-of-scope "Link to this heading")
...
## 2. Verified current state (facts, fetched — not assumed)[#](#verified-current-state-facts-fetched-not-assumed "Link to this heading")
...
### Page-1 source titles (verified by fetch — the only titles confirmed so far)[#](#page-1-source-titles-verified-by-fetch-the-only-titles-confirmed-so-far "Link to this heading")
...
## 3. The Rules[#](#the-rules "Link to this heading")
...
### 3.1 RST / Sphinx structural rules[#](#rst-sphinx-structural-rules "Link to this heading")
...
### 3.2 Per-term entry contract (the canonical shape of one term)[#](#per-term-entry-contract-the-canonical-shape-of-one-term "Link to this heading")
...
### 3.3 sphinx\_design component patterns (copy-paste, do not improvise)[#](#sphinx-design-component-patterns-copy-paste-do-not-improvise "Link to this heading")
...
### 3.4 sphinx\_tags rules (corrected to match project convention)[#](#sphinx-tags-rules-corrected-to-match-project-convention "Link to this heading")
...
* ****Move the tag block to the bottom**** (just above or just below `Sources`). The current draft places it near the top (≈ line 40) — that contradicts the convention. (sphinx\_tags renders the badge wherever the directive sits; if you want a **visual** level indicator at the top, use `sd-badge` from sphinx\_design instead of the tags directive.)
...
### 3.5 Sources rules (verified-only)[#](#sources-rules-verified-only "Link to this heading")
...
### 3.6 Content-integrity rules (no hallucination, no copying)[#](#content-integrity-rules-no-hallucination-no-copying "Link to this heading")
...
### 3.7 UX rules (“Discovery at a glance” + nested exploration)[#](#ux-rules-discovery-at-a-glance-nested-exploration "Link to this heading")
...
## 4. The Master Taxonomy (the home for all 431 terms)[#](#the-master-taxonomy-the-home-for-all-431-terms "Link to this heading")
...
## 5. Gap analysis[#](#gap-analysis "Link to this heading")
...
## 6. Title harvest method (deterministic, reproducible)[#](#title-harvest-method-deterministic-reproducible "Link to this heading")
...
### 6.1 Primary method — category pagination (confirmed to work)[#](#primary-method-category-pagination-confirmed-to-work "Link to this heading")
...
### 6.2 Secondary method — RSS feed (spot-check / freshness)[#](#secondary-method-rss-feed-spot-check-freshness "Link to this heading")
...
### 6.3 Optional — WordPress REST API (only if the host exposes it)[#](#optional-wordpress-rest-api-only-if-the-host-exposes-it "Link to this heading")
...
### 6.4 Output of the harvest[#](#output-of-the-harvest "Link to this heading")
...
## 7. Multi-run roadmap (one batch per session)[#](#multi-run-roadmap-one-batch-per-session "Link to this heading")
...
### Run 0 — Structure & hygiene (fast, do first)[#](#run-0-structure-hygiene-fast-do-first "Link to this heading")
...
### Run 1 — Harvest batch A + Domain 4/10 reconciliation[#](#run-1-harvest-batch-a-domain-4-10-reconciliation "Link to this heading")
...
### Run 2 — Harvest batches B–D (pages 11–44)[#](#run-2-harvest-batches-bd-pages-1144 "Link to this heading")
...
### Runs 3–9 — Content fill, one domain per run[#](#runs-39-content-fill-one-domain-per-run "Link to this heading")
...
### Run 10 — Final pass[#](#run-10-final-pass "Link to this heading")
...
### 7.3 Placeholder convention (for not-yet-written content)[#](#placeholder-convention-for-not-yet-written-content "Link to this heading")
...
## 8. Integration checklist[#](#integration-checklist "Link to this heading")

### 8.1 Wire the page into the parent toctree (REQUIRED — currently missing)[#](#wire-the-page-into-the-parent-toctree-required-currently-missing "Link to this heading")
...
### 8.2 Build & verify[#](#build-verify "Link to this heading")
...
### 8.3 Cross-link hygiene[#](#cross-link-hygiene "Link to this heading")
...
## 9. Verified sources (consulted for this guide)[#](#verified-sources-consulted-for-this-guide "Link to this heading")
...
* Documentation Tagging Guidelines: https://scikit-plots.github.io/dev/devel/guide\_document\_tag.html
...
## 10. Quick rule card (pin this)[#](#quick-rule-card-pin-this "Link to this heading")
