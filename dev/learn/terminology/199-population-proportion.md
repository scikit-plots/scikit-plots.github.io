...
# Population Proportion[#](#population-proportion "Link to this heading")
...
Since you rarely measure everyone, you estimate p with the ****sample proportion**** \(\hat{p} = x/n\)
(successes over sample size). \(\hat{p}\) is a ****statistic**** that varies from sample to sample, with
****standard error**** \(\sqrt{p(1-p)/n}\) shrinking as ****n**** grows — the basis for ****confidence intervals****
