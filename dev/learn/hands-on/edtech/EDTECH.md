# BilimEdtech Labs mirror — course guide (Mode B, verbatim CC BY 4.0)[#](#bilimedtech-labs-mirror-course-guide-mode-b-verbatim-cc-by-4-0 "Link to this heading")
...
## What is in this folder[#](#what-is-in-this-folder "Link to this heading")
...
## Upstream structure and fetch-map[#](#upstream-structure-and-fetch-map "Link to this heading")
...
## How the generator works[#](#how-the-generator-works "Link to this heading")
...
### The Source & license admonition (every mirrored page)[#](#the-source-license-admonition-every-mirrored-page "Link to this heading")
...
### Extending / editing later (the whole point of the generator)[#](#extending-editing-later-the-whole-point-of-the-generator "Link to this heading")
...
## Fetch strategy (proven)[#](#fetch-strategy-proven "Link to this heading")
...
### Bulk crawl — `fetch_edtech.py` (fast path, run locally)[#](#bulk-crawl-fetch-edtech-py-fast-path-run-locally "Link to this heading")
...
### Fetch landmines (seen in practice)[#](#fetch-landmines-seen-in-practice "Link to this heading")
...
## Canonical URL / duplicate-content (SEO) — the one hook outside RST[#](#canonical-url-duplicate-content-seo-the-one-hook-outside-rst "Link to this heading")
...
## RST & validation landmines specific to this mirror[#](#rst-validation-landmines-specific-to-this-mirror "Link to this heading")
...
* ****Code-block example roles / directives (RST-workshop pages) — handled.**** Pages
  that **teach** RST embed example roles like `:ref:`genindex`and example directives like `.. toctree::` / `.. include::` inside literal/code blocks. Real Sphinx treats code-block contents as literal and ignores them. Both tools here now do the same via a shared `_mask_code_blocks` helper that blanks literal/code-block bodies (a `.. code-block::`/`code`/`sourcecode`/ `parsed-literal`/`literalinclude` directive, or a line ending in `::` that is not itself a directive, followed by its blank-or-more-indented body) **and blanks inline-literal spans** (double backticks), so an example role written inline like ````:ref:`Label``` ``` is also ignored. `\_buildcheck.py`masks before the`:doc:`/`:ref:`scan, and`build\_edtech.py`masks before the`toctree`scan (so an example`.. toctree::`inside a code block does **not** spawn spurious stub pages — this is exactly what created the phantom`web-guide/index`and`…`stubs before the fix). The masker only *skips* checks/parsing, so it cannot regress existing courses (all remained green with identical shas). Real`.. automodule::`(Task 9) still must never appear as a live directive — reproduce it only inside a code block; and`.. image::`targets need a real file present (see`PLACEHOLDER\_IMAGES`).

## Validation & rebuilding[#](#validation-rebuilding "Link to this heading")
...
### Current mirror status[#](#current-mirror-status "Link to this heading")
...
* ****Validator anchor collection**** now matches Sphinx: explicit labels are collected
  ...
  `:ref:`text <Step 5.3>```). The pattern `^\s*\.\. _([^:\n]+):\s*$` still excludes `.. \_name: url`` external-link targets. Propagated to all 11 drop-in
...
* ****One pending cross-section reference:**** `writing-rst-8` (Tables) links to the
  cloud-computing OpenVPN lab via ``:ref:`Step 1. Initialize OpenVPN using Docker```. That label lives on a not-yet-mirrored cloud-computing page, so the reference is validator-green (space-form, so the `:ref:` check skips it) but will
...
## Delivery / sync (nested folder note)[#](#delivery-sync-nested-folder-note "Link to this heading")
