# [Step 2: Configure Nginx using the Command Line](#id5)[#](#step-2-configure-nginx-using-the-command-line "Link to this heading")
...
## [2.1. Create sub-domain ****chat.example.com****](#id6)[#](#create-sub-domain-chat-example-com "Link to this heading")

We need our sub-domain name that points to our VPS before our reverse
proxy will work.

> **Tip**
> Here are the instructions for Freenom and Namecheap.
You will have to find the instructions if you used a different
registrar.

### [Freenom](#id7)[#](#freenom "Link to this heading")

[Freenom.com](https://www.freenom.com) is a source for free domains names from
TLDs .tk, .ml, ga, .cf, and .gq.

1. Add your subdomain names.

![../../../../../_images/freenom5.png](../../../../../_images/freenom5.png)
![../../../../../_images/freenom6.png](../../../../../_images/freenom6.png)
![../../../../../_images/freenom7.png](../../../../../_images/freenom7.png)

* You might have to wait several hours if you’re unlucky

### [Namecheap](#id8)[#](#namecheap "Link to this heading")

See the [knowledgebase article from Namecheap](https://www.namecheap.com/support/knowledgebase/article.aspx/9776/2237/how-to-create-a-subdomain-for-my-domain) if you used their services.
...
3. Click the green checkmark or press the **Save all changes** button
4. Wait several minutes before trying to ping the new sub-domain name.

   * You might have to wait several hours if you’re unlucky![../../../../../_images/chat-domain-name.png](../../../../../_images/chat-domain-name.png)

## [2.2. Create the Nginx Site](#id9)[#](#create-the-nginx-site "Link to this heading")
...
1. Create the new Nginx config file and copy the file contents.
   ...
   * Press ****y**** to save the changes
   ...
   ```
   File contents of /etc/nginx/sites-available/chat.example.com[#](#id1 "Link to this code")
   ```
    1server {
    2    listen 80;
    3
...
2. Enable the Nginx Site
   ...
   > 1. `sites-available` contains that sites that are configured
   >    but not active.
   > 2. `sites-enabled` contains the sites that are active and
   ...
   ```
3. Verify that there are no configuration issues
   ...
   ```
4. Restart Nginx and verify that it restarted without errors
   ...
   ```
5. Now, let’s verify that the reverse proxy is working correctly.
   We will use `curl` to access the site using the domain name.

   * The file is very large.
   * If something is wrong if you see the default Nginx HTML.Expected Output[#](#id4 "Link to this code")
   ```
   ...
   .
   .
   configCalled%22%3Atrue%2C%22isModern%22%3Afalse%7D"))</script>

     <script type="text/javascript" src="/498130460737d5eb424ed320e4abde4f744cfe8b.js?meteor_js_resource=true"></script>

   </body>
   </html>root@vps298933:~#

   ```
6. Lastly, we should add the SSL certificate using `certbot`.
   ...
   * Let’s promote a secure web!
   ...
   >
   > ```
7. If everything is successful, you should be able to access your
   chat application using your web browser.

   * Here is the URL to my box: <http://chat.y.jj8i.com>
8. Navigate to the URL using your web browser and configure Rocket.Chat
