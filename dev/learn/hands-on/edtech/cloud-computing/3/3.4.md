# [Step 4: Add additional Services](#id9)[#](#step-4-add-additional-services "Link to this heading")

Let’s add another service to our project. Wordpress stores most of its
data in the database, which can slow the system down if the database
is hit frequently. We will improve performance by using memory caching.
The first time a request is made, the data is pulled from the database
and stored in memory. If the same data is accessed again, the caching
service uses the copy in memory.

[Redis](https://redis.io) and [Memcached](https://memcached.org/) are both in-memory data storage systems.
Memcached is a high-performance distributed memory cache service,
and Redis is an open-source key-value store. Similar to Memcached,
Redis stores most of the data in memory. These services keep data in
memory because it is faster than reading it from the database.
We’ll add [Redis from Docker Hub](https://hub.docker.com/_/redis).

## [Install Redis Server](#id10)[#](#install-redis-server "Link to this heading")

> **Tip**
> Students often times have mistakes in their YAML file because of
incorrect spaces. You can validate your `docker-compose.yml` file
using website [YAML Lint](http://www.yamllint.com/).

1. Add the Redis service to our Wordpress `docker-compose.yml` file.

   1. Edit file `docker-compose.yml`.
   2. Add a new section called `redis` between sections `db`
      and `wordpress`.
   ```
   redis:
     image: redis
...
3. Let’s restart the project and configure Redis.
   ...
   * Use `&&` is to separate the commands
   ...
   ```
   * As you can see, Docker Compose downloaded image `redis:latest`
     and then created a new container called `wordpressdocker_redis_1`.
   * We can’t test Redis easily because we did not configure an outside port.
     You can see that it is running on an internal port only.
     Other Docker containers can access it, but we can’t.Output[#](#id2 "Link to this code")
   ```
...
## [Testing Redis on Docker](#id11)[#](#testing-redis-on-docker "Link to this heading")

You can explore this more on your own, but you can test the Redis server
running on container `wordpressdocker_redis_1` if you know the IP address
...
1. Locate the IP address of the container using
   ****docker inspect****:
   ...
   ```
   > **Note**
   > Your IP address will be different. The IPAddress key value
   pair in the example below i:

   ```
   "IPAddress": "192.168.208.2",

   ```
   IP Address of Container[#](#id3 "Link to this code")
   ```
   ...
   ```
2. Now we can test Redis using `redis-cli` because Redis is
   available on port 6379. But, we need to install the package first.
   ...
   ```
3. First of all, let’s test the connection using `ping`.
   ...
   ```
   > **Error**
   > * You will get a `Connection refused` message if something
     is misconfigured or if you have the wrong IP address,
     such as trying to use localhost.Output[#](#id6 "Link to this code")
   ```
   root@vps298933:~/wordpress-docker# redis-cli ping
...
4. We can use the redis-cli tool to look at all of the information available.
   ...
   ```
   * We can look at individual sections of the info that are separated
     by the ****#**** symbol, such as `server` or `memory`.
   ```
