# [Step 2: Modifying a Docker Image](#id4)[#](#step-2-modifying-a-docker-image "Link to this heading")

****Objective****: Modify a docker to include updated packages.

One problem with projects on GitHub is that they get stale.
A developer built the project for their current need but then
didn’t maintain it.

> **Tip**
> We should modify Docker projects with up-to-date packages.

## [4.2.1 Updating old Code](#id5)[#](#updating-old-code "Link to this heading")

The version of Node in the [pandoc-as-a-service](https://github.com/mrded/pandoc-as-a-service) Dockerfile has a
long list of published [security vulnerabilities](https://www.cvedetails.com/vulnerability-list/vendor_id-12113/product_id-30764/version_id-218775/Nodejs-Node.js-6.10.0.html). We will replace
Node 6.10 with the [latest version of Node](https://hub.docker.com/_/node) (v 15.2) in DockerHub.

1. Edit the pandoc-as-a-service `Dockerfile` and replace the
   version of Node.
   ...
   ```
2. Let’s try to rebuild. It will fail because we need to make
   additional changes, but we’ll do them one at a time.
...
3. `npm audit` found a problem!
   ...
   ```
4. The fix is simple. We need to append `npm audit fix --force`
   to the `RUN` command.
...
5. Save the file and then rerun the same build command to fix the issues.
   ...
   ```
6. We can also update the HTML by modifying `views/index.ejs`.
   For example, we updated the URLs in the code on our
   [pandoc.bilimedtech.com](https://pandoc.bilimedtech.com/) site.

## [4.2.2. Cleanup Failed Builds](#id6)[#](#cleanup-failed-builds "Link to this heading")

Docker will tag an image as `<none>` anytime the build fails.

Failed Builds[#](#id1 "Link to this code")
```
...
## [4.2.3 Relaunch with the updated Images](#id7)[#](#relaunch-with-the-updated-images "Link to this heading")

The final step is to update the `docker-compose.yml` file with
...
1. Change to the project directory, update the image name, and then restart
   ...
   ```
   docker-compose.yml[#](#id3 "Link to this code")
   ```
    version: "3.3"
...
2. Verify that the new image is in use.
   ...
   ```
    ...
    9aaf93a4aa5b        pandoc:latest       "docker-entrypoint.s…"   38 seconds ago      Up 35 seconds       0.0.0.0:20852->8080/tcp   pandocdocker_pandoc_1
    1a13f235f757        nextcloud           "/entrypoint.sh apac…"   10 hours ago        Up 10 hours         0.0.0.0:20850->80/tcp     nextclouddocker_app_1
    ...
    cd832a14a92c        wordpress:latest    "docker-entrypoint.s…"   23 hours ago        Up 23 hours         0.0.0.0:20851->80/tcp     wordpressdocker_wordpress_1
...
Source page:
<https://labs.bilimedtech.com/cloud-computing/4/4.2.html>
