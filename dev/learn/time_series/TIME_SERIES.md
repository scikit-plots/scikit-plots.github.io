# scikit-plots `learn/time_series` — Regeneration & Maintenance Guide[#](#scikit-plots-learn-time-series-regeneration-maintenance-guide "Link to this heading")
...
## 0. TL;DR — rebuild in one command[#](#tl-dr-rebuild-in-one-command "Link to this heading")
...
## 1. What lives in this folder[#](#what-lives-in-this-folder "Link to this heading")
...
## 2. Architecture — how the pieces fit[#](#architecture-how-the-pieces-fit "Link to this heading")
...
2. ****`STAGES` + `GLOSS`**** (dicts inside `build_time_series.py`) — `STAGES[key] = (emoji, title, level, blurb)` defines the six course stages; `GLOSS[title]` is the one-line summary shown on
   each hub card.
...
### The 6 stages (in order)[#](#the-6-stages-in-order "Link to this heading")
...
## 3. Regenerating the pages (the build)[#](#regenerating-the-pages-the-build "Link to this heading")
...
## 4. Validating (run after every change — mandatory)[#](#validating-run-after-every-change-mandatory "Link to this heading")
...
## 5. Updating & extending (add or edit lessons)[#](#updating-extending-add-or-edit-lessons "Link to this heading")

### Edit an existing lesson[#](#edit-an-existing-lesson "Link to this heading")

1. Edit `CONTENT["<exact title>"]` (and/or `MINDMAP["<exact title>"]`) in `ts_content.py`.
2. `python build_time_series.py` → 3. validate.

### Add a new lesson[#](#add-a-new-lesson "Link to this heading")
...
### The exact-title rule (critical)[#](#the-exact-title-rule-critical "Link to this heading")
...
### Batch cadence[#](#batch-cadence "Link to this heading")
...
## 6. Page content contract (what a good lesson contains)[#](#page-content-contract-what-a-good-lesson-contains "Link to this heading")
...
## 7. RST / formatting conventions (what keeps the build clean)[#](#rst-formatting-conventions-what-keeps-the-build-clean "Link to this heading")
...
## 8. Regenerating from scratch (if you start with nothing but this guide)[#](#regenerating-from-scratch-if-you-start-with-nothing-but-this-guide "Link to this heading")
...
1. ****Harvest all 18 titles deterministically**** by paginating the category
   (`/category/introduction-to-time-series/page/N/`, ~10 posts/page → 2 pages). Record `title <TAB> url`.
...
## 9. Deterministic sync + zip (only if publishing to a separate output dir)[#](#deterministic-sync-zip-only-if-publishing-to-a-separate-output-dir "Link to this heading")
...
## 10. Fail-fast guard & invariants (the safety net)[#](#fail-fast-guard-invariants-the-safety-net "Link to this heading")
...
## 11. Suggestions & future work[#](#suggestions-future-work "Link to this heading")
...
## 12. Quick rule card (pin this)[#](#quick-rule-card-pin-this "Link to this heading")
...
## Hub layout (v2 — filterable browser)[#](#hub-layout-v2-filterable-browser "Link to this heading")
...
* one ****`.. dropdown::` per stage**** (6 of them), titled with the stage emoji, number, name
...
* one ****A–Z master dropdown**** listing all 18 lessons alphabetically (auto-sorted);
* ****new stable anchors****: `_ts-stage-<key>:` before each stage dropdown (linkable from
...
`99c739754b32af806d3a186120e2c72c5915119b520ae14e5ec2392455564bfd`. Everything else in this guide is
