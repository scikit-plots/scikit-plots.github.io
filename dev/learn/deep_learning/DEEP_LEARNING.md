# scikit-plots `learn/deep_learning` — Regeneration & Maintenance Guide[#](#scikit-plots-learn-deep-learning-regeneration-maintenance-guide "Link to this heading")
...
2. ****`STAGES` + `GLOSS`**** (dicts inside `build_deep_learning.py`) — `STAGES[key] = (emoji, title, level, blurb)` defines the four course stages; `GLOSS[title]` is the one-line summary shown on
   each hub card.
...
### The 4 stages (in order)[#](#the-4-stages-in-order "Link to this heading")
...
* ****Math**** uses `.. math::` (display) or `:math:`…`(inline), LaTeX syntax. Inline code (numpy calls) uses double back-ticks:`np.dot(w.T, X)``.
...
### Curriculum map (the 17 lessons, in order)[#](#curriculum-map-the-17-lessons-in-order "Link to this heading")
