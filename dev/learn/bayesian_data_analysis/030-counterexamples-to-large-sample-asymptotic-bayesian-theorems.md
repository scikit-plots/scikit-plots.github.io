# Counterexamples to large-sample (asymptotic) Bayesian theorems[#](#counterexamples-to-large-sample-asymptotic-bayesian-theorems "Link to this heading")
...
## At the boundary[#](#at-the-boundary "Link to this heading")

If the true value sits on the ****edge**** of the parameter space, the posterior cannot be normal there: it
has nowhere to put mass on one side. A hierarchical variance \(\tau^2\) whose true value is
...
The normal approximation on a ****transformed**** scale (\(\log \tau\)) helps, but the boundary case is
inherently awkward.

## Not identified, or growing[#](#not-identified-or-growing "Link to this heading")
...
number of parameters ****grows with**** \(n\) — one per observation, as in the classic
Neyman–Scott problem — consistency for the parameters of interest can fail outright, since new data
bring new unknowns.

## Infinite dimensions[#](#infinite-dimensions "Link to this heading")
...
## Misspecification, the everyday case[#](#misspecification-the-everyday-case "Link to this heading")
...
Credible intervals shrink like \(1/\sqrt{n}\) around a value that is not the truth, growing more
