# Further extensions to Gibbs and Metropolis[#](#further-extensions-to-gibbs-and-metropolis "Link to this heading")
...
that a hard move in \(\theta\) becomes an easy move in \((\theta, u)\).

## Slice sampling[#](#slice-sampling "Link to this heading")

Introduce a height \(u\) beneath the unnormalised density and sample ****uniformly from the area under
the curve****. Given \(\theta\), draw \(u \sim \mathrm{Uniform}(0, q(\theta))\); given \(u\),
draw \(\theta\) uniformly from the ****slice**** \(\{\theta : q(\theta) \ge u\}\). Marginally,
\(\theta\) has the target distribution.
...
## Reversible jump[#](#reversible-jump "Link to this heading")
...
## Other auxiliary-variable tricks[#](#other-auxiliary-variable-tricks "Link to this heading")

* ****Data augmentation**** — latent variables that restore conjugacy: a \(t\) as a scale-mixture of
...
## Where they matter now[#](#where-they-matter-now "Link to this heading")
