# Example: parallel experiments in eight schools[#](#example-parallel-experiments-in-eight-schools "Link to this heading")
...
scores. Each school reported an estimated effect \(y\_j\) and its standard error \(\sigma\_j\),
both taken as given:

| School | A | B | C | D | E | F | G | H |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| Effect \(y\_j\) | 28 | 8 | -3 | 7 | -1 | 1 | 8 | 12 |
| Std. error \(\sigma\_j\) | 15 | 10 | 16 | 11 | 9 | 11 | 10 | 18 |

School A’s effect of 28 points looks dramatic. Its standard error is 15. Should we believe it?

## Three analyses[#](#three-analyses "Link to this heading")

****No pooling**** takes each \(y\_j\) at face value: school A’s effect is 28. But the standard errors
...
## What the data say about \(\tau\)[#](#what-the-data-say-about-tau "Link to this heading")

The pivotal quantity is \(\tau\), the ****between-school**** standard deviation. Its posterior is
...
\(B\_j = \sigma\_j^2/(\sigma\_j^2 + \tau^2)\) are large, and school A’s estimate is pulled from 28 down
...
## The funnel[#](#the-funnel "Link to this heading")

Fit the ****centred**** version (\(\theta\_j \sim \mathrm{N}(\mu, \tau)\) directly) and the sampler
reports ****divergences****. The reason is geometric: as \(\tau \to 0\) the \(\theta\_j\) are squeezed
into an ever-narrowing neck, and no single step size explores both the neck and the wide mouth. The
****non-centred**** parameterisation above — sampling standard-normal \(\eta\_j\) and rescaling —
