# Hamiltonian Monte Carlo[#](#hamiltonian-monte-carlo "Link to this heading")

****Part 3 · Stage 9 · ⛓️ MCMC: Gibbs, Metropolis & HMC**** · Lesson 078 of 144 · **intermediate**

[◀ Previous · Further extensions to Gibbs and Metropolis](077-further-extensions-to-gibbs-and-metropolis.html) · [Next · Hamiltonian Monte Carlo for a hierarchical model ▶](079-hamiltonian-monte-carlo-for-a-hierarchical-model.html) · [↑ Section](index.html)
...
## Stop wandering; use the gradient[#](#stop-wandering-use-the-gradient "Link to this heading")

Random-walk Metropolis explores by diffusion, needing \(\mathcal{O}((L/c)^2)\) steps to travel a
distance \(L\). ****Hamiltonian Monte Carlo**** replaces the aimless jump with a ****directed trajectory****,
...
## Physics on the posterior[#](#physics-on-the-posterior "Link to this heading")

Treat \(\theta\) as the position of a particle on a landscape whose ****potential energy**** is the
negative log posterior. Add an auxiliary ****momentum**** \(\rho\), drawn afresh each iteration from
\(\mathrm{N}(0, M)\). The total energy is the ****Hamiltonian****

\[H(\theta, \rho) = \underbrace{-\log p(\theta \mid y)}\_{\text{potential}}
+ \underbrace{\tfrac{1}{2} \rho^{\top} M^{-1} \rho}\_{\text{kinetic}},\]

and the joint density \(p(\theta, \rho) \propto e^{-H(\theta, \rho)}\) has the posterior as its
\(\theta\)-marginal — so simulating the particle’s motion and ****discarding the momentum**** samples the
posterior. Hamiltonian flow conserves \(H\), is time-reversible, and preserves phase-space volume:
exactly the properties a valid MCMC proposal needs.

## Leapfrog, and the correction[#](#leapfrog-and-the-correction "Link to this heading")
...
residual error is removed by a Metropolis accept/reject on \(e^{-H}\), whose acceptance is near one
...
## Divergences: not a warning, a wound[#](#divergences-not-a-warning-a-wound "Link to this heading")
...
> **Hint**
> ****Related lessons:**** [Metropolis and Metropolis-Hastings algorithms](070-metropolis-and-metropolis-hastings-algorithms.html) · [Hamiltonian Monte Carlo for a hierarchical model](079-hamiltonian-monte-carlo-for-a-hierarchical-model.html) · [Stan: developing a computing environment](080-stan-developing-a-computing-environment.html) · [Inference and assessing convergence](072-inference-and-assessing-convergence.html)
> **See also**
> ****Source article**** Adapted (context, re-expressed) in our own words from: <https://insightful-data-lab.com/2025/11/22/hamiltonian-monte-carlo/> (insightful-data-lab.com).
