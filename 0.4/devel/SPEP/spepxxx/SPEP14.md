# SEP14: Text handling[#](#sep14-text-handling "Link to this heading")
...
Of the above options, it should be noted that [harfbuzz](https://github.com/harfbuzz/harfbuzz) is designed
...
[WebKit](https://webkit.org/), and possibly whether than
...
`rcParams[“font.family”]` (default: `['sans-serif']`), but the same is not possible with “usetex”. It may be
...
fonts. The default is to use the font list cache in `font_manager`
which matches fonts using our own algorithm based on the [CSS font
matching algorithm](http://www.w3.org/TR/CSS2/fonts.html#algorithm).
It doesn’t always do the same thing as the native font selection
algorithms on Linux ([fontconfig](https://www.freedesktop.org/wiki/Software/fontconfig/)), Mac and
...
There is also experimental support for using [fontconfig](https://www.freedesktop.org/wiki/Software/fontconfig/) for font
...
Linux, we already have the bones of a [fontconfig](https://www.freedesktop.org/wiki/Software/fontconfig/) implementation
...
Cairo), or using [fontforge](https://fontforge.org) (which is a heavy and not terribly
...
rcParam to choose the text engine (“text.layout\_engine”), but it
...
(fontname, glyph\_id) and the values are the paths so that only one
...
1. Rewriting the freetype wrapper, and removing ttconv.
   ...
   1. Once (1) is done, as a proof of concept, we can move to the
