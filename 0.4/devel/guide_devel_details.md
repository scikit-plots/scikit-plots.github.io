# Development Details Guidelines[#](#development-details-guidelines "Link to this heading")

> **Template**
> Template for further usage, template belong to [astropy](https://docs.astropy.org/en/latest/development/development_details.html#).

This document contains somewhat-independent sections that provide details on
topics covered in the [Quickstart Contributing Guidelines](guide_qu_contribute.html#quickstart-contributing) guide.
...
Or if you prefer [conda](https://docs.conda.io/en/latest/):
...
[install guide](https://pre-commit.com/#install). Once this installation is
...
you with fixing any issues with your code style, see [Fixing coding style issues](maintainers/maintainer_workflow.html#pre-commit-bot) for details
on how to use this bot.

## The editing workflow[#](#the-editing-workflow "Link to this heading")
...
3. Add documentation to your code and, as appropriate, to the Astropy
   documentation.
4. Ideally, also make sure your changes do not break the documentation.
5. Add tests of the code you contribute.
6. Commit your changes in [git](https://git-scm.com/).
7. Repeat as necessary.

### In more detail[#](#in-more-detail "Link to this heading")

1. Make some changes to one or more files. You should follow the Astropy
   [Coding Guidelines](guide_code.html#code-guide). Each logical set of changes should be treated as one
...
2. Test that your changes do not lead to **regressions**, i.e. that your
   changes do not break existing code, by running the Astropy tests
   as described in [Testing Guidelines](guide_test.html#testing-guidelines).
3. Make sure your code includes appropriate docstrings, in the
   ...
   a detailed description is in [Documentation Guidelines](guide_document.html#documentation-guidelines).
4. If you have sphinx installed, you can also check that
   ...
   any warnings. (For more details, see [Documentation Guidelines](guide_document.html#documentation-guidelines).)
5. Add tests of your new code, if appropriate. Some changes (e.g. to
   documentation) do not need tests. Detailed instructions are at
   [Writing tests](guide_test.html#writing-tests), but if you have no experience writing tests or
   with the [pytest](https://docs.pytest.org/en/stable/) testing framework submit your changes without adding
   tests, but mention in the pull request that you have not written tests.
   An example of writing a test is in
   [Stop and think: Any more tests or other changes?](git_edit_workflow_examples.html#astropy-fix-add-tests).
6. Stage your changes using `git add` and commit them using `git commit`.
   An example of doing that, based on the fix for an actual Astropy issue, is
   at [Contributing Code: a Worked Example](git_edit_workflow_examples.html#astropy-fix-example).

   > **Note**
   > Make your [git](https://git-scm.com/) commit messages short and descriptive. If a commit
...
7. Some modifications require more than one commit; if in doubt, break
...
## Add a changelog entry[#](#add-a-changelog-entry "Link to this heading")
...
An example entry, for the changes in [PR 1845](https://github.com/astropy/astropy/pull/1845), the file would be
...
## Make a pull request[#](#make-a-pull-request "Link to this heading")
...
a pull request](https://docs.github.com/en/pull-requests/collaborating-with-pull-requests/proposing-changes-to-your-work-with-pull-requests/creating-a-pull-request), or follow the steps below.

When you are ready to ask for someone to review your code and consider merging
it into Astropy:

1. Go to the URL of your fork of Astropy, e.g.,
   `https://github.com/your-user-name/astropy`.
2. Use the ‘Switch Branches’ dropdown menu to select the branch with your
   changes:

   ![../_images/branch_dropdown.png](../_images/branch_dropdown.png)
3. Click on the ‘Pull request’ button:
   ...
   For usage of pre-commit hooks and directives, see [Pre-commit](#pre-commit) and [Fixing coding style issues](maintainers/maintainer_workflow.html#pre-commit-bot).
...
resolve conflicts as necessary. See [Rebase if necessary](#rebase) and [How to rebase](#howto-rebase)
for further information.

## Rebase if necessary[#](#rebase-if-necessary "Link to this heading")
...
### When to rebase[#](#when-to-rebase "Link to this heading")
...
### How to rebase[#](#how-to-rebase "Link to this heading")

It is easier to make mistakes rebasing than other areas of [git](https://git-scm.com/), so before you
...
Behind the scenes, [git](https://git-scm.com/) is deleting the changes and branch you made, making the
...
rebase](https://help.github.com/en/articles/resolving-merge-conflicts-after-a-git-rebase).

## Squash if necessary[#](#squash-if-necessary "Link to this heading")

Squashing refers to combining multiple commits into a single commit. This can be done
using the `git rebase` command or via [Github ‘Squash and merge’](maintainers/maintainer_workflow.html#github-squash-and-merge).

An Astropy maintainer will be happy to guide you through this process.

### When to squash[#](#when-to-squash "Link to this heading")
...
### How to squash[#](#how-to-squash "Link to this heading")

In many cases, squashing can be done by a maintainer using the [Github ‘Squash and merge’](maintainers/maintainer_workflow.html#github-squash-and-merge) button on
the GitHub pull request page. If this is not possible, we typically squash using [git
rebase –interactive](https://git-scm.com/docs/git-rebase#_interactive_mode). In
...
## How to push[#](#how-to-push "Link to this heading")
...
discussing of rebasing is at [Rebasing on main](guide_git_resources.html#rebase-on-trunk).
...
## Troubleshooting the build[#](#troubleshooting-the-build "Link to this heading")
...
## External C Libraries[#](#external-c-libraries "Link to this heading")
...
* [wcslib](https://www.atnf.csiro.au/people/mcalabre/WCS/) see
  `cextern/wcslib/README` for the bundled version. To use the
  system version, set `ASTROPY_USE_SYSTEM_WCSLIB=1`.
* [expat](https://libexpat.github.io/) see `cextern/expat/README` for the
